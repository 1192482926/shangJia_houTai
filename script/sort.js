...
jQuery(document).ready(function(){
	...
	//删除提示
	jQuery(".order-list .operating a.del").live("click",function(){
		var _name = jQuery(this).parents(".tr").find(".package-name").text();
		var popuptitle = "温馨提示";//弹窗标题
		var popupcontent = "<p>您确定要详细类目“<em>"+_name+"</em>”吗？</p>";//弹窗内容
		var btns = "2";//按钮数量
		var backfunction = "DeleteSort";//调用函数
	...
	jQuery(".main .btns .delete").on("click",function(){
		...
		for(var i=0;i < _checkbox ;i++){
			if(jQuery(".tbody .checkbox").eq(i).is(":checked")){
				_name += "“<em>"+jQuery(".tbody .checkbox").eq(i).parents(".tr").find(".package-name").text()+"</em>”";
				n = n + 1 ;
			}
		}
		if(n>0){
			var popuptitle = "温馨提示";//弹窗标题
			var popupcontent = "<p>您确定要详细类目"+_name+"吗？</p>";//弹窗内容
			var btns = "2";//按钮数量
